...
HEALTH_FONT = pygame.font.SysFont("roboto", 30)
...
# Code to handel yellow spaceship movements 
def handle_yellow_movements(key_pressed, yellow):
    ...
    if key_pressed[pygame.K_s] and yellow.y + VEL + yellow.height < HEIGHT - SPACESHIP_MARGIN:  # DOWN
        yellow.y += VEL

# Code to handel red spaceship movements
def handle_red_movements(key_pressed, red):
    if key_pressed[pygame.K_LEFT] and red.x - VEL > BORDER.x + 20:  # LEFT
        red.x -= VEL
            
    if key_pressed[pygame.K_RIGHT] and red.x + VEL < WIDTH - 30:  # RIGHT
        red.x += VEL
        
    if key_pressed[pygame.K_UP] and red.y - VEL > 0: # UP
        red.y -= VEL

    if key_pressed[pygame.K_DOWN] and red.y + VEL + red.height < HEIGHT - SPACESHIP_MARGIN:  # DOWN
        red.y += VEL   
        
def handel_bullets(yellow_bullets, red_bullets, yellow, red):
    # Loop to handel yellow bullets
    for bullet in yellow_bullets[:]:
...
def main():
    ...
    while run:
        ...
        if winner_text != "":
            draw_winner(winner_text)  # Some has won
            run = False
        
        key_pressed = pygame.key.get_pressed()
        handle_yellow_movements(key_pressed, yellow)
        handle_red_movements(key_pressed, red)
